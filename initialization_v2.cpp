...
void Play1()
{
	...
	inputnum = 2;
	outputnum = 0;
	input[0].num = '1'; input[1].num = '2';
	input[0].filled=1;  input[1].filled=1;
	output[0].num=output[1].num=0;
	output[0].filled=output[1].filled=0;
	for(int i=2;i<10;i++)
	{
		input[i].num=0;
		input[i].filled=0;
...
void PrintWood(int x, int y, bool e, int number)  //e for is_empty ,避免变量名重合直接用e
{
	COORD coord = { 0 };		//光标结构体定义
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 5; j++)
...
void movetoleft()
{
	...
	if (current.filled)
	{
		...
		PrintWood(nowx - 1 + 1, nowy - 3, current.filled, current.num);
	}
	nowx--;
	current_coord.X = nowx - 1 + 1;
	current_coord.Y = nowy - 3;
	initrobot(nowx, nowy);
	PrintRobot();
	Sleep(100);
}
...
void PrintInwood()
{
	...
	for (int i = 0; i < 6; i++)
	{
		PrintWood(inwood.body[i].X, inwood.body[i].Y, input[i].filled, input[i].num);
	}
}
//inwood   x=5,y=1
void PrintEmptywood()
{
	...
	for (int i = 0; i < 4; i++)
	{
		PrintWood(emptywood.body[i].X, emptywood.body[i].Y, space[i].filled, space[i].num);
...
void PrintOutwood()
{
	...
	for (int i = 0; i < 4; i++)
	{
		PrintWood(outwood.body[i].X, outwood.body[i].Y, output[i].filled, output[i].num);
	}
	return;
}
//outwood    x=40,y=1
